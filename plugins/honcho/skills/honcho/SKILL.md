...
You are about to bring up **honcho** from a *recipe*, not from a finished instance. The
skill ships honcho as source: a carried recipe plus the pin to its harness, like a
self-extracting archive that carries both its contents and the unpacker. Before you tell
anyone honcho is running, HOIST it against **this** target and GRADE it honestly. This is
...
Two roles run through this. The **author** wrapped honcho as this skill on their machine.
The **receiver** is whoever installs it — a different target, whose substrate, secrets,
and paths you resolve here. You work so the receiver reaches a real, graded honcho — or an
honest reason it could not, never a silent success.

## Hoist recipe (run before your first report that honcho is up)

1. **Cache / idempotency.** If honcho is already hoisted on this target in this session (a
...
4. **Know early, then deploy.** Run preflight first — it deploys nothing. If it says
   ...
   teardown. You never re-run honcho's own singular deployment onto a live host.
...
7. **Hand off.** Hand the user to the operators the carried recipe includes (develop,
   sysop, petard) so they can *exploit* honcho — extend it, operate it, keep it running
...
- "Reusing the honcho hoist from earlier this session. Ready."

The failure to avoid: reporting honcho is up because the recipe *looks* right. It is up only
...
- `health-reports-ok`: `curl -fsS http://localhost:8000/health | grep -qi ok`
- `openapi-served`: `curl -fsS http://localhost:8000/openapi.json | grep -q '"openapi"'`
- `workspace-crud-roundtrip`: `curl -fsS -X POST http://localhost:8000/v3/workspaces -H 'Content-Type: application/json' -d '{"name":"hoist-smoke"}' | grep -q hoist-smoke`
