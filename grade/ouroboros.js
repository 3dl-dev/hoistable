...
const setup = await agent(
  [
    ...
    "   python3 -c \"import sys;sys.path.insert(0,'release');import build_release;print(build_release.build('grade','<work>/kit',['envelope','hoist','operators','builder','release']))\"",
    '2. Derive a resolvable pin from that kit with builder/emit.py pin_for_kit (a file:// URL to',
    '   the kit path is fine; the receivers run on this machine).',
    '3. For each app, emit its skill with builder/emit.py emit_skill(config, pin) and write it to',
